...
<!-- Modal Backdrop -->
<div id="{{ $name }}-backdrop" class="modal-backdrop" style="display:none;" aria-hidden="true"></div>

<!-- Modal Content -->
<div 
    id="{{ $name }}" 
    class="modal-content w-full {{ $maxWidthClass }}" 
    style="display:none;"
    role="dialog"
    aria-modal="true"
    aria-labelledby="{{ $title ? $name . '-title' : '' }}"
    tabindex="-1"
>
    <div class="rounded-2xl shadow-2xl border" style="background-color: var(--bg-card); border-color: var(--border-color);">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b" style="border-color: var(--border-color);">
            <h3 id="{{ $name }}-title" class="text-xl font-semibold" style="color: var(--text-primary);">{{ $title }}</h3>
            <button 
